...
{
    [SerializeField] bool breakOnStart = false;
    [SerializeField] bool breakFragmentsToo = false;
    [SerializeField] bool hasFiniteLifetime = true;
    [ConditionalField(nameof(hasFiniteLifetime))] [SerializeField] float fragmentLifetime = 20;
    [SerializeField] bool hasParticleEffect;
    [ConditionalField(nameof(hasParticleEffect))] [SerializeField] float particleLifetime = 10;
    [ConditionalField(nameof(hasParticleEffect))] [SerializeField] GameObject particleEffectPrefab;
    [SerializeField] int fragmentCount = 5;
    [SerializeField] float explodeForce = 3000;

    bool edgeSet = false;
    Vector3 edgeVertex = Vector3.zero;
    Vector2 edgeUV = Vector2.zero;
    Plane edgePlane = new Plane();
    LayerMask meshBreakerMask;

    private void Start()
    {
        meshBreakerMask = LayerData.instance.meshBreakerMask;
        if (breakOnStart)
            Break();
    }
    ...
    public void Break()
    {
        this.SendMessageUpwards("Breaked", SendMessageOptions.DontRequireReceiver);
        ...
        for (var i = 0; i < parts.Count; i++)
        {
            ...
            parts[i].GameObject.layer = meshBreakerMask.GetFirstLayer();
            //if (hasParticleEffect)
            //    parts[i].GameObject.AddComponent<ParticleBreak>().ParticlePrefab = particleBreak.ParticlePrefab;
            parts[i].GameObject.GetComponent<Rigidbody>().AddForceAtPosition(parts[i].Bounds.center * explodeForce, transform.position);
        }

        if (hasParticleEffect)
        {
            GameObject fx = Instantiate(particleEffectPrefab, transform.position, Quaternion.identity);
    ...
    public class PartMesh
    {
        ...
        public void MakeGameobject(MeshBreaker original)
        {
            ...
            if (original.breakFragmentsToo)
            {
                var meshBreaker = GameObject.AddComponent<MeshBreaker>();
                meshBreaker.fragmentCount = original.fragmentCount;
                meshBreaker.explodeForce = original.explodeForce;
                meshBreaker.hasParticleEffect = original.hasParticleEffect;
                meshBreaker.fragmentLifetime = original.fragmentLifetime;
                meshBreaker.particleLifetime = original.particleLifetime;
                if (original.hasParticleEffect)
                    meshBreaker.particleEffectPrefab = original.particleEffectPrefab;
            }

            if (original.hasFiniteLifetime)
                Destroy(GameObject, original.fragmentLifetime);
        }
